...
MYELOID_FEATS = ['CD11b', 'CD11c', 'CD14', 'CD141', 'CD16', 'CD19', 'CD1c', 'CD3',
       'CD56', 'CD66b', 'FSC', 'Fixable Aqua', 'HLA', 'SSC']


class LongCovidDataset(InMemoryDataset):
    def __init__(
        self,
        root: str,
        name: str = 'longcovid',
    ...
    @property
    def feats_names(self)->List[str]:
        if self.variant == "myeloid":
            return MYELOID_FEATS
    ...
    def download(self):
        ...
        data_path = '/gpfs/gibbs/pi/krishnaswamy_smita/hsteach/long_COVID/current_data/meld_continuous_prepross/'
        import pickle

        with open(os.path.join(data_path, 'filt_datasets.pk'), 'rb') as file:
            filt_dsets = pickle.load(file)

        data = filt_dsets[self.variant][f"data_{self.variant}"]
        clinical = filt_dsets[self.variant][f"clinical_{self.variant}"].set_index("record_num")
        ...
        for pat_id in np.unique(list(data.index)):
            len_pat = data.loc[pat_id].shape[0]

            for n in range(n_samples):
                idx = np.random.choice(np.arange(len_pat),size=n_points, replace = False)
                subsampled_pcs.append(data.loc[pat_id].iloc[idx].values)
                subsampled_patient_ids.append(pat_id)
                
                label_ = clinical.loc[pat_id]['s1_medprov_longcovid']
...
class LongCovidSelectTransform(object):
...
class DummyLongCovidSelectTransform(object):
    def __init__(self):
        return
...
class LongCovidData(pl.LightningDataModule):
    def __init__(self, variant, 
        ...
        dataset_alias = f"longcovid_{variant}" #@param ["ModelNet10", "ModelNet40"] {type:"raw"}
        #self.feats = feats
        self.variant = variant
        ...
        base_pre_transform = [T.NormalizeScale(), DummyLongCovidSelectTransform()]
        ...
        gss = StratifiedGroupKFold(n_splits = 2, random_state = random_state, shuffle = True)
        gss_test = StratifiedGroupKFold(n_splits = 2, random_state = random_state, shuffle = True)

        train_idx, val_idx = next(gss.split(np.arange(len(dataset)), dataset.patient_classes, groups = dataset.patient_labels))
        val_idx, test_idx = next(gss_test.split(val_idx, np.array(dataset.patient_classes)[val_idx], groups = np.array(dataset.patient_labels)[val_idx]))
