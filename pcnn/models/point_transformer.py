...
class TransformerBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.pos_nn = MLP([30, 64, out_channels], norm=None, plain_last=False)
...
class point_transformer(pl.LightningModule):
    def __init__(self, input_dim, num_classes,lr, dim_model=[32, 64, 128, 256, 512], k=16, **kwargs):
        super().__init__()
        self.k = k
        self.lr = lr
    ...
    def forward(self, data, batch=None):
        ...
        if data.x is None:
            x = torch.ones((data.pos.shape[0], 1), device=data.pos.get_device())

        # first block
        x = self.mlp_input(data.x)
